...
> 当前状态：阶段 2 射击闭环 + 阶段 3 手感设置（DPI / 灵敏度 / FOV / yaw；HUD 显示开镜 180° cm）。
...
浏览器打开 Vite 提示的地址（默认 http://localhost:5173）。点击画面锁定指针；左键用准星中心射击；下方可改手感后点应用；`1` 红点，`2` 切 2 倍，`Esc` 退出锁定。
