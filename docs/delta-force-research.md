...
| --- | --- | --- | --- |
| `redDot` | 1.0 | 110°（若手感偏慢/画面偏近，再按 1.25x 试） | ADS 附件；游戏内「1x」往往仍略放大 |
| `scope2x` | 2.0 | ≈ 71.1° | 2 倍镜 Scope 管线 |

再转垂直 FOV，套 MDV 1.33。1x 与 2x 必须分预设，训练器提供切换，默认 **红点**。
...
训练默认对齐**开镜 MDV**。开镜预设：**红点（精校默认）** 与 **2 倍镜（精校默认）**，可切换。红点若画面仍明显放大，把 zoom 从 1.0 调到 1.25 再对靶场。
