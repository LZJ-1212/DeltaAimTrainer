...
相机 `fov` 用 **开镜垂直 FOV**，不是腰射 110。

机主主用 **红点 + 2 倍**，精校不改。mouse-sensitivity.com 把「红点/全息」和「同倍率 Scope」分成两条 FOV 管线，数字不能混用。

名义光学 FOV（镜内放大关、精校默认、腰射 110H 16:9）先用：

```
hAds ≈ 2 * atan(tan(hHip / 2) / zoom)
```

| 预设 | zoom | 水平 FOV 初值 | 说明 |
| --- | --- | --- | --- |
| `redDot` | **1.25**（精校默认） | ≈ 93.3° | 社区称出厂红点是 1.25x，不是 1.0；[Reddit](https://www.reddit.com/r/DeltaForceGlobal/comments/1ticfc8/can_we_please_have_an_option_to_change_the_scope/) |
| `scope2x` | 2.0 | ≈ 71.1° | 2 倍镜 Scope 管线 |

再转垂直 FOV，套 MDV 1.33。1x 与 2x 必须分预设，训练器提供切换，默认 **红点（1.25x）**。

精确 FOV 以靶场为准：开镜后看同一参照物占屏比例，或对照游戏 180°/甩到屏幕边缘的垫上距离。
...
训练默认对齐**开镜 MDV**。开镜预设：**红点（出厂约 1.25x）** 与 **2 倍镜（精校默认）**，可切换。
